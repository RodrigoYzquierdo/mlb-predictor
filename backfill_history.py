...
# Cargar pesos del modelo entrenado v3.0 (mismo que usa el script diario)
MODEL_WEIGHTS = None
try:
    with open("public/model_weights.json") as f:
        MODEL_WEIGHTS = json.load(f)
    print(f"Modelo v3.0 cargado: {MODEL_WEIGHTS['n_games']} partidos, "
          f"{round(MODEL_WEIGHTS['cv_accuracy']*100,1)}% cross-val")
except Exception as e:
    print(f"model_weights.json no disponible, usando heuristico: {e}")
...
def calc_model(h_abbr, a_abbr, standings, team_stats):
    ...
    # === MODELO v3.0: regresion logistica entrenada ===
    if MODEL_WEIGHTS:
        features = [
            h_wp - a_wp,                                          # dif_winpct
            (h_rdiff - a_rdiff) / 100.0,                          # dif_rundiff
            (at["era"] - ht["era"]) * pf_pitching_adj,            # dif_era
            (at["bullpen_era"] - ht["bullpen_era"]) * pf_pitching_adj,  # dif_bullpen
            (ht["ops"] - at["ops"]) * pf_hitting_adj,             # dif_ops
        ...
        hp = max(25, min(75, round(prob_home * 100)))
        return hp
    ...
    hs  += (5.0 - min(ht["bullpen_era"]* pf_pitching_adj, 5.5)) * 5
...
def main():
    ...
    for i, date in enumerate(dates):
        ...
        # 1. Calcular prediccion de TODOS los partidos del dia
        day_preds = []
        for g in games:
            ...
            day_preds.append({
                ...
                "hp": hp, "pred": fav, "actual": score, "hit": hit, "diff": diff,
            })

        # 2. Ordenar por conviccion y tomar SOLO el Top 5 del dia
        top5_dia = sorted(day_preds, key=lambda x: x["diff"], reverse=True)[:5]

        # 3. Guardar esos 5 (sin el campo diff, que es interno)
        for g in top5_dia:
